...
function sourceMethodForProgram(program: number): DiscoveryMethod {
  switch (program) {
    ...
    case 5: return "BOOTSCAN";
    case 6: return "SISCAN";
    case 8: return "3SEQ";
    default: return "RDP";
  }
}
...
async function runScan(request: Extract<WorkerRequest, { type: "scan" }>): Promise<ScanResults> {
  ...
  try {
    ...
    const terminalProgress = emitProgress(true);
    const adaptedResults = sourceFaithfulCore && (rawResults as SourceFaithfulResult).sourceFaithfulCore
      ? makeSourceFaithfulResults(rawResults as SourceFaithfulResult, request.options, dataset)
      : rawResults as ScanResults;
    const results = sourceFaithfulCore && terminalProgress
      ? {
          ...adaptedResults,
          processedTriplets: terminalProgress.processedTriplets,
          totalTriplets: terminalProgress.totalTriplets,
          correctionTests: terminalProgress.correctionTests,
          cumulativeTriplets: terminalProgress.cumulativeTriplets,
          scanRounds: Math.max(1, terminalProgress.scanRound),
          maximumDetectionCycles: terminalProgress.maximumDetectionCycles,
          cycleTermination: terminalProgress.cycleTermination,
        }
      : adaptedResults;
